...
def load_dataset():
    """Load dataset from hard-coded file path"""
    try:
        with open(DATASET_PATH, 'r') as file:
            return [line.strip() for line in file if line.strip()]
    except FileNotFoundError:
        st.error(f"Error: Dataset file not found at {DATASET_PATH}")
        st.stop()
...
def save_feedback():
    ...
    try:
        with open(FEEDBACK_PATH, 'w') as f:
            json.dump(list(st.session_state.feedback.values()), f)
...
# Chat input
if prompt := st.chat_input("Ask about cats..."):
    if not st.session_state.vector_db:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Searching cat facts..."):
